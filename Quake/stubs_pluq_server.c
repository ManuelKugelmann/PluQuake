...
// stubs_pluq_server.c -- Server-side stubs for PluQ frontends
// Provides stubs for server/network/QuakeC functionality not needed by frontends
// Used by BOTH test and production frontends
...
// Map/mod management stubs (maptype_t is Ironwail-specific, use int for QuakeSpasm)
const char *ExtraMaps_GetMessage (const filelist_item_t *item) { return ""; }
int ExtraMaps_GetType (const filelist_item_t *item) { return 0; }
qboolean ExtraMaps_IsStart (int type) { return false; }
...
int Modlist_GetStatus (const filelist_item_t *item) { return 0; } // modstatus_t is Ironwail-specific
...
globalvars_t *pr_global_struct = NULL;
...
void PR_PushQCVM (void *newvm, void **oldvm) { if (oldvm) *oldvm = NULL; }
void PR_PopQCVM (void *oldvm) {}
string_t PR_SetEngineString (const char *s) { return 0; }
const char *PR_GetString (string_t num) { return ""; }
void PR_ClearProgs (void *vm) {}
...
// Save/load stubs (use void* for QuakeSpasm compatibility - savedata_t is Ironwail-specific)
void SaveData_Clear (void *save) {}
void SaveData_Init (void *save) {}
void SaveData_WriteHeader (void *save) {}
void SaveData_Fill (void *save) {}
